        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center justify-start rtl:justify-end">
                ...
                <a href="/" class="flex items-center">
                    <img src="/images/logo_sprigatito.jpg" class="ml-3 md:ml-0 w-16 rounded-full" alt="Lisahwan Logo" />
                </a>
            </div>
    ...
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-900">
        <ul class="space-y-2 font-medium">
            {{-- @auth
                @if (Auth::user()->isOwner()) --}}
                    <li>
                        <a href="{{ route('list_transaksi') }}"
                            class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-700 group">
                            <svg class="{{ $active_1 ?? 'text-green-500' }} flex-shrink-0 w-5 h-5 transition duration-75 group-hover:{{ $active_1 ?? 'text-gray-100' }}"
            ...
            {{-- @endauth --}}
            <li>
                <a href="{{ route('list_transaksi') }}" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-700 group">
